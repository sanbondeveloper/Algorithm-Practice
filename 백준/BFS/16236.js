...
while (true) {
  // bfs 함수는 먹을 수 있는 경우 true, 없는 경우 false를 반환
  if (bfs()) {
    count += 1;
    board[posX][posY] = 0;

    // 먹은 횟수과 아기 상어의 크기가 같으면 +1
    if (count === size) {
      size += 1;
      count = 0;
    }
...
/*
 - N의 크기가 크지 않아 그래프 탐색을 하는 데 큰 무리는 없다.
 - BFS로 최단거리를 찾되 최단거리에 있는 물고기 중 우선순위가 높은 것을 선택해야 함
   - eat 변수 사용
   - 최단거리(time)가 같은 건 모두 좌표를 비교 
*/
